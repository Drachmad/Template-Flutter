...
import 'package:flutterpj/model/transaksi/model_memo.dart';
...
class MemoController with ChangeNotifier {
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_memo_list = await m_order.data_memo_paginate(
        cari, tanggal_awal, tanggal_akhir, offset, limit);
    var count =
        await m_order.count_memo_paginate(cari, tanggal_awal, tanggal_akhir);
  ...
  //     header_detail_excel.add("kreditTotal");
  //     List data_account = await m_order.select_memo_detail(
  //         data_memo_list[index_terpilih]['no_bukti'], "NO_BUKTI", "memo");
    ...
    await m_order
      ...
      if (value != null) {
        ...
        nobuktiController.text =
            "BL/LRI/${format_no_bukti.format(DateTime.now())}-${no_urut + 1}";
      }
    });
  ...
  Future<List> baca_tabeld() async {
    ...
    for (int i = 0; i < data_account_keranjang.length; i++) {
      ...
      double debetTotal = debet;
      double kreditTotal = kredit;
      Map obj = new Map();
      ...
      // obj['sub_total'] = kreditTotal ?? 0.00;
      accountList.add(obj);
    }
    return accountList;
  }
