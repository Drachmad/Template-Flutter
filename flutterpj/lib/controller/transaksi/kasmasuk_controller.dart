...
import 'package:flutterpj/model/transaksi/model_kasmasuk.dart';
...
class KasmasukController with ChangeNotifier {
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_kasmasuk_list = await m_order.data_kasmasuk_paginate(
        cari, tanggal_awal, tanggal_akhir, offset, limit);
    var count = await m_order.count_kasmasuk_paginate(
  ...
  void proses_export() {
    if (data_kasmasuk_list.length > 0) {
      ...
      for (int i = 0; i < data_kasmasuk_list.length; i++) {
        Map<String, dynamic> isi_map = new Map<String, dynamic>();
        isi_map['a'] = data_kasmasuk_list[i]['tanggal'];
        isi_map['b'] = data_kasmasuk_list[i]['no_bukti'];
        isi_map['c'] = data_kasmasuk_list[i]['sales'];
        isi_map['d'] = data_kasmasuk_list[i]['customer'];
        isi_map['e'] = data_kasmasuk_list[i]['keterangan'];
        isi_map['f'] = data_kasmasuk_list[i]['total_qty'];
        isi_map['g'] = data_kasmasuk_list[i]['total_so'];
        if (data_kasmasuk_list[i]['status'] == 1) {
          isi_map['h'] = "Diterima";
        } else {
  ...
  Future<void> proses_export_detail() async {
    if (index_terpilih != null) {
      ...
      map_transaksi['a'] = data_kasmasuk_list[index_terpilih]['tanggal'];
      map_transaksi['b'] = data_kasmasuk_list[index_terpilih]['no_bukti'];
      map_transaksi['c'] = data_kasmasuk_list[index_terpilih]['sales'];
      map_transaksi['d'] = data_kasmasuk_list[index_terpilih]['customer'];
      map_transaksi['e'] = data_kasmasuk_list[index_terpilih]['keterangan'];
      if (data_kasmasuk_list[index_terpilih]['status'] == 1) {
      ...
      List data_account = await m_order.select_kasmasuk_detail(
          data_kasmasuk_list[index_terpilih]['no_bukti'], "NO_BUKTI", "kas");
      ...
      footer_excel.add(data_kasmasuk_list[index_terpilih]['total_qty']);
      footer_excel.add("Total");
      footer_excel.add(data_kasmasuk_list[index_terpilih]['total_so']);
      String judul =
          "Invoice Order Penjualan (${data_kasmasuk_list[index_terpilih]['no_bukti']})";
  ...
  Future<void> proses_print() async {
    ...
    //     .proses_print(data_kasmasuk_list[index_terpilih], data_account);
  }
