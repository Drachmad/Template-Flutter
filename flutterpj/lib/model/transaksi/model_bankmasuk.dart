...
class model_bankmasuk {
  ...
  ///paginate
  Future count_bankmasuk_paginate(
      String key_cari, String start_date, String end_date) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/count_bankmasuk_paginate"),
  ...
  ///SELECT HEADER
  Future<List> data_bankmasuk_paginate(String cari, String start_date,
      String end_date, int paramoffset, int paramlimit) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/bankmasuk_paginate"),
      body: {
        "cari": cari,
        "tglawal": start_date,
        "tglakhir": end_date,
